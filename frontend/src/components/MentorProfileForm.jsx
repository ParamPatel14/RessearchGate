  ...
  const renderViewMode = () => (
    <div className="animate-fade-in space-y-8">
      ...
      <div className="relative bg-[var(--color-academia-charcoal)] rounded-xl p-8 text-[var(--color-academia-cream)] shadow-lg overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-xl opacity-90 font-medium font-sans">
              {formData.mentor_type === 'academic_supervisor' 
                ? `${formData.university} ${formData.lab_name ? `• ${formData.lab_name}` : ''}`
                : formData.company}
            </p>
            {formData.website_url && (
              <a href={formData.website_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-4 text-[var(--color-academia-gold)] hover:text-[var(--color-academia-cream)] transition-colors">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[var(--color-academia-gold)] text-[var(--color-academia-charcoal)] px-6 py-3 rounded-lg font-bold shadow-lg hover:bg-[var(--color-academia-gold-hover)] hover:scale-105 transition-all duration-200"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-200 flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
          <span className="text-stone-500 text-xs uppercase tracking-wider font-semibold mb-1">Students</span>
          <span className={`text-lg font-bold ${formData.accepting_phd_students === 'Yes' ? 'text-green-600' : 'text-[var(--color-academia-charcoal)]'}`}>
            {formData.accepting_phd_students === 'Yes' ? 'Accepting' : formData.accepting_phd_students}
          </span>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-200 flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
          <span className="text-stone-500 text-xs uppercase tracking-wider font-semibold mb-1">Funding</span>
          <span className={`text-lg font-bold ${formData.funding_available === 'Yes' ? 'text-green-600' : 'text-[var(--color-academia-charcoal)]'}`}>
            {formData.funding_available}
          </span>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-200 flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
          <span className="text-stone-500 text-xs uppercase tracking-wider font-semibold mb-1">Reputation</span>
          <div className="flex items-center gap-1 text-[var(--color-academia-gold)] font-bold text-lg">
            <FiAward /> {user?.mentor_profile?.reputation_score || "N/A"}
          </div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-200 flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
          <span className="text-stone-500 text-xs uppercase tracking-wider font-semibold mb-1">Outcomes</span>
          <span className="text-lg font-bold text-[var(--color-academia-charcoal)]">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[var(--color-academia-charcoal)] rounded-xl shadow-lg p-6 text-[var(--color-academia-cream)]">
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.preferred_backgrounds ? (
                    formData.preferred_backgrounds.split(',').map((bg, i) => (
                      <span key={i} className="text-xs bg-white/10 px-2 py-1 rounded text-[var(--color-academia-cream)] border border-white/20">
                        {bg.trim()}
                      </span>
                    ))
              ...
              <div>
                <h4 className="text-xs uppercase tracking-wider text-stone-400 font-semibold mb-2">Minimum Expectations</h4>
                <ul className="text-sm space-y-2 text-stone-300">
                  {formData.min_expectations ? (
                    formData.min_expectations.split(',').map((exp, i) => (
                      <li key={i} className="flex items-start gap-2">
                        <span className="text-[var(--color-academia-gold)] mt-1">•</span> {exp.trim()}
                      </li>
                    ))
               ...
               {formData.application_requirements && (
                  <div className="pt-4 border-t border-white/10">
                    <h4 className="text-xs uppercase tracking-wider text-stone-400 font-semibold mb-2">Application Req.</h4>
                    <p className="text-sm text-stone-300">{formData.application_requirements}</p>
                  </div>
               )}
            </div>
          </div>
           ...
           <div className="p-6 bg-[var(--color-academia-charcoal)] text-[var(--color-academia-cream)] flex justify-between items-center sticky top-0 z-20 shadow-md">
            ...
            {user?.mentor_profile && (
                <button 
                    ...
                    className="text-[var(--color-academia-cream)]/80 hover:text-[var(--color-academia-cream)] hover:bg-white/10 px-4 py-2 rounded-lg transition-colors text-sm font-medium"
                >
                    Cancel
                </button>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-8 max-w-5xl mx-auto">
              ...
              <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {formData.mentor_type === 'academic_supervisor' ? (
                        <>
                            <div>
                                <label className="block text-sm font-medium text-stone-700 mb-1">University / Institution</label>
                                <input
                                    type="text"
                                    name="university"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-stone-700 mb-1">Position / Title</label>
                        <input
                            type="text"
                            name="position"
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <FiGlobe className="absolute left-3 top-3 text-stone-400" />
                            <input
                                type="url"
                                name="website_url"
                ...
                <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        <div>
                         ...
                         <div>
                            <label className="block text-sm font-medium text-stone-700 mb-1">Funding Available?</label>
                            <select
                        ...
                        <div>
                            <label className="block text-sm font-medium text-stone-700 mb-1">Max Requests/Month</label>
                            <input
                                type="number"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-stone-700 mb-1">Mentorship Style</label>
                            <select
                                ...
                                {backgrounds.map(bg => (
                                    <label key={bg} className="inline-flex items-center space-x-2 cursor-pointer hover:bg-white p-1 rounded transition">
                                        <input
                                            ...
                                            className="form-checkbox text-[var(--color-academia-gold)] rounded focus:ring-[var(--color-academia-gold)] h-4 w-4"
                                        />
                                        <span className="text-sm text-stone-700">{bg}</span>
                                    </label>
                                ))}
                            </div>
                                ...
                                {expectations.map(exp => (
                                    <label key={exp} className="inline-flex items-center space-x-2 cursor-pointer hover:bg-white p-1 rounded transition">
                                        <input
                                            ...
                                            className="form-checkbox text-[var(--color-academia-gold)] rounded focus:ring-[var(--color-academia-gold)] h-4 w-4"
                                        />
                                        <span className="text-sm text-stone-700">{exp}</span>
                                    </label>
                                ))}
                            </div>
                             <div className="mt-3 pt-3 border-t border-stone-200">
                        ...
                        {publications.map((pub, index) => (
                            <div key={index} className="bg-stone-50 p-4 rounded-lg border border-stone-200 relative group transition-all hover:shadow-md">
                                <button
                                    ...
                                    className="absolute top-2 right-2 text-stone-400 hover:text-red-500 p-1 transition-colors"
                                    title="Remove"
                                >
                                    <FiTrash2 />
                                </button>
              ...
              <div className="flex justify-end pt-4">
                ...
                >
                  {isLoading ? (
                    <>
                        <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-[var(--color-academia-gold)]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
